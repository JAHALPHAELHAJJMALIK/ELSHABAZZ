...
CREATE  OR ALTER PROCEDURE dbo.[CHPIV_sp_PALLIATIVE_UM_CLAIMS] -- <Procedure_Name, sysname, ProcedureName> ... DROP PROCEDURE ... EXEC dbo.[CHPIV_sp_PALLIATIVE_UM_CLAIMS]; 
...
SET @lobplanprog = 'CHPIV' -- bp.[OPTION(S)]: IN ('COMMUNITY Y MAS (HMO C-SNP)','DSNP MEDI-CAL PLAN','MEDI-CAL BENEFIT PLAN') ... p.[OPTION(S)]: IN ('DSNP MEDI-CAL','DSNP MEDICARE','COMMUNITY Y MAS (HMO C-SNP)','MEDI-CAL') -- ACTIVE LOB LOCK ... 
SET @PalliativeDx ='Z51.5'

		-- SELECT value FROM STRING_SPLIT(@SPLITlob, ',')

		/* SELECT ' ' AS '"DECLARE*" @PARAM(S)'
		,'BETWEEN '+CAST(CAST(@RangeStartDate AS date) AS varchar(255))+' AND '+CAST(CAST(@RangeEndDate AS date) AS varchar(255)) AS 'RANGE NOTE(s)',bp.*
...
WHERE 1=1
	AND bp.[LINE_OF_BUSINESS] LIKE ISNULL('%'+@lobplanprog+'%','%') -- CARDINALITY HIERARCHY CASUALTY ... LEAVE AS FINAL FILTER
	AND bp.[BRAND] IN (ISNULL(@BRAND,'CHPIV')) -- ('MEDI-CAL BENEFIT PLAN','DSNP MEDI-CAL PLAN','COMMUNITY Y MAS (HMO C-SNP)','IV_DSNP2 
...
SELECT 'CHPIV' AS 'Parent Company Name' -- MS EXCEL ODBC: TEMPLATE
,'#####/D-SNP' AS 'H Contract # '
