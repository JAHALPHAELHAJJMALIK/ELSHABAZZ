...
FROM #AdmitAssessment AS aa
	...
	WHERE 1=1 
		AND claimid IN (SELECT DISTINCT claimid FROM INFORMATICS.dbo.DHCS_LTC_CHPIV)) AS t -- DO NOT NO NEGATIVE <> != COVER UP THE TE() ... 50 YEAR CALENDAR [sp]
...
FROM #AdmitAssessment AS aa
	...
	-- SELECT TOP 10 ' ' AS 'CHECK 1st',*
	-- SELECT DISTINCT ' ' AS 'CHECK 1st'
	FROM INFORMATICS.dbo.date_calendarISO AS dc,(SELECT DISTINCT affiliationid,memid,startdate,enddate,dischargedate,provid FROM HMOPROD_PLANDATA.dbo.claim WHERE claimid IN (SELECT DISTINCT claimid FROM INFORMATICS.dbo.DHCS_LTC_CHPIV)) AS t -- DO NOT NO NEGATIVE <> != COVER UP THE TE() ... 50 YEAR CALENDAR [sp]
...
FROM #AdmitAssessment AS aa
	-- JOIN INFORMATICS.dbo.DHCS_LTC_CHPIV AS developsource ON aa.AdmitID = developsource.AdmitID
WHERE 1=1 
...
FROM INFORMATICS.dbo.DELIVERABLE_DHCS_LTC_CHPIV AS s
	JOIN INFORMATICS.dbo.DHCS_LTC_CHPIV AS d ON s.memid = d.memid
WHERE 1=1
...
CROSS APPLY (SELECT DISTINCT [RANGE NOTE(S)] FROM INFORMATICS.dbo.DHCS_LTC_CHPIV) AS rp -- INDEPENDENT STAND ALONE ... CROSS APPLY() CROSS JOIN() CARTESIAN PRODUCT
