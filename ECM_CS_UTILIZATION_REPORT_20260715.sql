...
-- C007: PER MS CLAUDIA'S 20260715 EMAIL REPLY (C-SUITE REVIEW) - "CS Template UCLA Survey.xlsx" PROVIDED AS THE COMPLETE / AUTHORITATIVE CS Service Name / HCPC Code / Modifier TABLE, STATED TO REPLACE THE PRIOR CODESET BECAUSE "the HCPC Code and Modifier combination fields were limited." BOTH #QUALIFYING_CLAIMS' CS Service Received GATE AND THE [Cx] POPULATE CASE (SEE C006) NARROWED TO EXACTLY THE 32 code+modifier ROWS / 8 DISTINCT MODIFIER-GROUPINGS IN HER TABLE, AND [Cx] LABELS SWITCHED TO HER EXACT [CS Service Name] COLUMN TEXT (TRIMMED ONLY, NOT OTHERWISE ALTERED). THIS REMOVES 6 CODE GROUPS FROM THE PRIOR (20260713) CODESET THAT ARE NOT PRESENT IN HER TABLE - PENDING WALTER / CLAUDIA SIGN-OFF BEFORE THIS BECOMES THE STANDING DEFINITION:
		--   REMOVED - H0040 (both U1 and U2 legs, prior codeset)
		--   REMOVED - H2014 (Day Habilitation leg, prior codeset)
		--   REMOVED - S9452 (both U5 and U6 legs, DHCS v1.3 Jan 2026 addition, prior M004)
		--   REMOVED - S9977 (all 5 legs U4/U5/U7/U8/U9, DHCS v1.3 Jan 2026 addition, prior M005)
		--   REMOVED - H0044+U5 (Non-Recurring Set-Up Expenses leg specifically - H0044 WITH U2/U3/U6 REMAINS VALID, only the U5 modifier combination is dropped)
		--   REMOVED - T1016+U8 (Housing Outreach - Initiation, DHCS-mandatory PACES reporting code per prior M006 change-log note "MCPs must report these... both successful and unsuccessful") - FLAGGED SPECIFICALLY: dropping this from THIS utilization report is likely fine for Claudia's narrower UCLA-survey purpose, but it is NOT the same as removing a PACES reporting obligation, needs an explicit yes from Walter/Claudia, not a silent removal
		-- QA check block (see 'TRUE Unmapped CS Codeset Leg') updated to exclude these 6 groups as a known/expected non-actionable pattern rather than a gap. Reporting window @RangeStartDate/@RangeEndDate also hardcoded below to the fixed 1/1/2026-6/30/2026 period per Claudia's "same reporting period" request, replacing the prior dynamic-YTD formula - see DECLARE/SET block.
...
SET @when = NULL
SET @RangeStartDate = CAST('01/01/2026' AS datetime) -- C007 20260715: HARDCODED PER MS CLAUDIA'S 20260715 REQUEST TO RE-RUN THE SAME REPORTING PERIOD, "1/1/26-6/30/26" - REPLACES PRIOR DYNAMIC-YTD FORMULA (WAS CAST('01/01/'+CAST(DATEPART(yyyy,DATEADD(month,@leadlag,CAST(GETDATE() AS datetime))) AS varchar(4)) AS datetime)) SO THIS RUN DOES NOT DRIFT TO THE CURRENT YEAR/MONTH
SET @RangeEndDate = CAST('06/30/2026' AS datetime) -- C007 20260715: HARDCODED, SAME REASON - WAS EOMONTH(GETDATE(),-1)
...
WHERE 1=1
	...
	AND 
	( -- INITIATE ... C007 20260715: NARROWED TO MS CLAUDIA'S "CS Template UCLA Survey.xlsx" 32-ROW CS Service Name / HCPC Code / Modifier TABLE, PER HER 20260715 EMAIL "I have provided all the CS Codes and Modifiers for all". THIS IS NOW THE AUTHORITATIVE CODESET FOR THIS REPORT, REPLACING THE FULLER DHCS v1.3 Jan 2026 (M004/M005/M006) CODESET BELOW - SEE C007 CHANGE.LOG AT TOP OF FILE FOR THE FULL LIST OF CODES DROPPED (H0040, H2014, S9452, S9977, T1016, H0044+U5) AND WHY THIS NEEDS WALTER/CLAUDIA SIGN-OFF BEFORE THIS BECOMES THE STANDING DEFINITION
	(SUBSTRING(UPPER(LTRIM(RTRIM(ISNULL(cdcpt.servcode,'')))),1,5) IN ('H0043','H2016','T2040','T2050','T2051','T2041','T2033','H0045','S5151','S9125','T2012','T2014','T2018','T2020','H2038','H2024','H2026','S5130','T1019','S5165','S5170','S9470','H0014','H0044') -- C007: Claudia's UCLA table, U6 leg group
		AND (cdcpt.modcode IN ('U6') OR cdcpt.modcode2 IN ('U6') OR cdcpt.modcode3 IN ('U6') OR cdcpt.modcode4 IN ('U6') OR cdcpt.modcode5 IN ('U6')))

	OR (SUBSTRING(UPPER(LTRIM(RTRIM(ISNULL(cdcpt.servcode,'')))),1,5) IN ('H0044') -- C007: Housing Deposit
		AND (cdcpt.modcode IN ('U2') OR cdcpt.modcode2 IN ('U2') OR cdcpt.modcode3 IN ('U2') OR cdcpt.modcode4 IN ('U2') OR cdcpt.modcode5 IN ('U2')))

	OR (SUBSTRING(UPPER(LTRIM(RTRIM(ISNULL(cdcpt.servcode,'')))),1,5) IN ('H0043','H0044') -- C007: Short-Term Post-Hospitalization
		AND (cdcpt.modcode IN ('U3') OR cdcpt.modcode2 IN ('U3') OR cdcpt.modcode3 IN ('U3') OR cdcpt.modcode4 IN ('U3') OR cdcpt.modcode5 IN ('U3')))

	OR (SUBSTRING(UPPER(LTRIM(RTRIM(ISNULL(cdcpt.servcode,'')))),1,5) IN ('T2038') -- C007: Nursing Facility Transition/Diversion to Assisted Living Facilities (RCF &ARF)
		AND (cdcpt.modcode IN ('U4') OR cdcpt.modcode2 IN ('U4') OR cdcpt.modcode3 IN ('U4') OR cdcpt.modcode4 IN ('U4') OR cdcpt.modcode5 IN ('U4')))

	OR (SUBSTRING(UPPER(LTRIM(RTRIM(ISNULL(cdcpt.servcode,'')))),1,5) IN ('H2022') -- C007: Nursing Facility Transition/Diversion to Assisted Living Facilities (RCF &ARF)
		AND (cdcpt.modcode IN ('U5') OR cdcpt.modcode2 IN ('U5') OR cdcpt.modcode3 IN ('U5') OR cdcpt.modcode4 IN ('U5') OR cdcpt.modcode5 IN ('U5')))

	OR (SUBSTRING(UPPER(LTRIM(RTRIM(ISNULL(cdcpt.servcode,'')))),1,5) IN ('T2038') -- C007: Community Transition Services/ Nursing Facility Transition to a Home
		AND (cdcpt.modcode IN ('U5') OR cdcpt.modcode2 IN ('U5') OR cdcpt.modcode3 IN ('U5') OR cdcpt.modcode4 IN ('U5') OR cdcpt.modcode5 IN ('U5')))

	OR (SUBSTRING(UPPER(LTRIM(RTRIM(ISNULL(cdcpt.servcode,'')))),1,5) IN ('S5165') -- C007: Asthma Remediation
		AND (cdcpt.modcode IN ('U5') OR cdcpt.modcode2 IN ('U5') OR cdcpt.modcode3 IN ('U5') OR cdcpt.modcode4 IN ('U5') OR cdcpt.modcode5 IN ('U5')))

	OR (SUBSTRING(UPPER(LTRIM(RTRIM(ISNULL(cdcpt.servcode,'')))),1,5) IN ('H0043') -- C007: Transitional Rent
		AND (cdcpt.modcode IN ('U2') OR cdcpt.modcode2 IN ('U2') OR cdcpt.modcode3 IN ('U2') OR cdcpt.modcode4 IN ('U2') OR cdcpt.modcode5 IN ('U2')))
		) -- CONCLUDE ...
		)
...
THEN
	CASE -- C007 20260715: RENAMED + NARROWED TO MS CLAUDIA'S "CS Template UCLA Survey.xlsx" TABLE - LABELS BELOW ARE HER EXACT [CS Service Name] COLUMN VALUES, TRIMMED OF LEADING/TRAILING WHITESPACE ONLY
	WHEN [CPT Service Code] = 'S5165'
		AND (modcode IN ('U6') OR modcode2 IN ('U6') OR modcode3 IN ('U6') OR modcode4 IN ('U6') OR modcode5 IN ('U6'))
	THEN 'Environmental Accessibility Adaptation (Home Modifications)'
	WHEN [CPT Service Code] = 'T2038'
		AND (modcode IN ('U4') OR modcode2 IN ('U4') OR modcode3 IN ('U4') OR modcode4 IN ('U4') OR modcode5 IN ('U4'))
	THEN 'Nursing Facility Transition/Diversion to Assisted Living Facilities (RCF &ARF)'
	...
	WHEN [CPT Service Code] = 'T2038'
		AND (modcode IN ('U5') OR modcode2 IN ('U5') OR modcode3 IN ('U5') OR modcode4 IN ('U5') OR modcode5 IN ('U5'))
	THEN 'Community Transition Services/ Nursing Facility Transition to a Home'
	...
	WHEN [CPT Service Code] IN ('H0045','S5151','S9125')
		AND (modcode IN ('U6') OR modcode2 IN ('U6') OR modcode3 IN ('U6') OR modcode4 IN ('U6') OR modcode5 IN ('U6'))
	THEN 'Respite Service'
	WHEN [CPT Service Code] IN ('T2012','T2014','T2018','T2020','H2038','H2024','H2026') -- C007 20260715: H2014 REMOVED - ABSENT FROM CLAUDIA'S TABLE, FLAGGED IN CHANGE.LOG FOR SIGN-OFF, NOT SILENTLY DROPPED
		AND (modcode IN ('U6') OR modcode2 IN ('U6') OR modcode3 IN ('U6') OR modcode4 IN ('U6') OR modcode5 IN ('U6'))
	THEN 'Day Habilitation'
	WHEN [CPT Service Code] IN ('S5130','T1019')
		AND (modcode IN ('U6') OR modcode2 IN ('U6') OR modcode3 IN ('U6') OR modcode4 IN ('U6') OR modcode5 IN ('U6'))
	THEN 'Personal Care and Homemaker Services'
	WHEN [CPT Service Code] IN ('S5170','S9470') -- C007 20260715: S9977 REMOVED FROM THIS BUCKET - ABSENT FROM CLAUDIA'S TABLE
		AND (modcode IN ('U6') OR modcode2 IN ('U6') OR modcode3 IN ('U6') OR modcode4 IN ('U6') OR modcode5 IN ('U6'))
	THEN 'Medically Tailored Meals/ Medically-Supportive Food'
	WHEN [CPT Service Code] = 'H0014'
		AND (modcode IN ('U6') OR modcode2 IN ('U6') OR modcode3 IN ('U6') OR modcode4 IN ('U6') OR modcode5 IN ('U6'))
	THEN 'Sobering Centers'
	WHEN [CPT Service Code] = 'H0043'
		AND (modcode IN ('U2') OR modcode2 IN ('U2') OR modcode3 IN ('U2') OR modcode4 IN ('U2') OR modcode5 IN ('U2'))
	THEN 'Transitional Rent'
	WHEN [CPT Service Code] = 'H0044'
		AND (modcode IN ('U6') OR modcode2 IN ('U6') OR modcode3 IN ('U6') OR modcode4 IN ('U6') OR modcode5 IN ('U6'))
	THEN 'Transitional Rent'
	ELSE 'Non-CS-Coded Line (Claim Classified CS Service Received)' -- C007 20260715: #QUALIFYING_CLAIMS' CS GATE WAS ALSO NARROWED TO CLAUDIA'S TABLE (SAME PASS, SEE ABOVE), SO H0040/H2014/S9452/S9977/T1016/H0044+U5 NO LONGER QUALIFY A CLAIM ON THEIR OWN - A CLAIM WHOSE ONLY CS-TYPE LINE WAS ONE OF THOSE 6 GROUPS NO LONGER REACHES THIS TABLE AT ALL (SEE C007 CHANGE.LOG FOR DOLLAR IMPACT, STILL PENDING WALTER/CLAUDIA SIGN-OFF). THIS ELSE BUCKET NOW ONLY CATCHES: (1) A GENUINE G9012/G9008 ECM LINE RIDING ON A CLAIM THAT ALSO HAS A STILL-QUALIFYING CS LINE (SEE C006B, UNCHANGED), OR (2) ONE OF THE 6 DROPPED CS CODE GROUPS RIDING AS A SIBLING LINE ON A CLAIM THAT ALSO HAS A STILL-QUALIFYING CS LINE. NEITHER IS A TRUE CODESET GAP - SEE 'TRUE Unmapped CS Codeset Leg' CHECK BELOW, WHICH EXCLUDES BOTH PATTERNS
	END
...
END
	...
	-- CHECK FOR TRUE UNMAPPED CS CODESET LEG(S) - SHOULD RETURN 0 ROWS --
	-- C007 20260715: EXCLUSION LIST EXPANDED - EXCLUDES BOTH THE ECM-ON-CS-CLAIM PATTERN (G9012/G9008, SEE C006B) AND THE 6 CODE GROUPS DROPPED PER CLAUDIA'S UCLA TABLE (H0040, H2014, S9452, S9977, T1016, H0044+U5). ANY ROW RETURNED HERE IS A GENUINE GAP - A code+modifier COMBINATION NOT ACCOUNTED FOR BY EITHER THE CS CODESET OR THE KNOWN ECM-ON-CS-CLAIM PATTERN - AND NEEDS INVESTIGATION, NOT JUST AN EXPECTED MIXED-LINE CLAIM
SELECT ' ' AS 'SHOULD RETURN 0 ROWS - TRUE Unmapped CS Codeset Leg'
,claimid
,claimline
,PAID_NET_AMT
,[InclusionExclusionType]
...
WHERE 1=1
	...
	AND [Cx] = 'Non-CS-Coded Line (Claim Classified CS Service Received)'
	AND NOT [CPT Service Code] IN ('G9012','G9008','H0040','H2014','S9452','S9977','T1016') -- NO NOT NEGATIVE <> != ... EXCLUDES ECM-ON-CS-CLAIM (G9012/G9008) PLUS 5 OF THE 6 DROPPED CODE VALUES
	AND NOT ( -- INITIATE ... EXCLUDES THE 6th DROPPED LEG, H0044+U5 (Non-Recurring Set-Up Expenses) - H0044 WITH U2/U3/U6 REMAINS A VALID CS LEG PER CLAUDIA'S TABLE, SO ONLY THE U5 COMBINATION IS EXCLUDED HERE
	[CPT Service Code] = 'H0044'
		AND (modcode IN ('U5') OR modcode2 IN ('U5') OR modcode3 IN ('U5') OR modcode4 IN ('U5') OR modcode5 IN ('U5'))
	) -- CONCLUDE ...
	-- AND claimid IN ('26106E08438','26072E10818') -- WALTER'S 20260713 SAMPLE CLAIMS - LEFT AS A COMMENTED REFERENCE, NOT AN ACTIVE FILTER

	-- QA REFERENCE ONLY - SIZE OF THE EXPECTED, NON-ACTIONABLE PATTERN(S) --
	-- C007 20260715: NOW SIZES BOTH THE ORIGINAL ECM-ON-CS-CLAIM PATTERN (G9012/G9008) AND THE NEWLY DROPPED CODE GROUPS RIDING AS SIBLING LINES ON A STILL-QUALIFYING CS CLAIM
SELECT ' ' AS 'QA REFERENCE - Non-Actionable Non-CS-Coded Lines On CS-Classified Claims'
,CASE
WHEN [CPT Service Code] IN ('G9012','G9008')
THEN 'ECM Code Riding On CS Claim (C006B)'
ELSE 'Dropped C007 CS Code Riding On CS Claim (H0040/H2014/S9452/S9977/T1016/H0044+U5)'
END AS [Pattern]
,COUNT(DISTINCT claimid) AS [Unique Claims]
...
END
	...
	-- QA REFERENCE ONLY - CLAIMS/DOLLARS THAT NO LONGER REACH THIS TABLE AT ALL UNDER THE NARROWED C007 GATE --
	-- RUN THIS AGAINST THE PRIOR (20260713) VERSION OF #QUALIFYING_CLAIMS' CS GATE, OR COMPARE THIS RUN'S TOTALS TO THE 20260713 VALIDATED FIGURES, TO SIZE THE FULL IMPACT OF DROPPING H0040/H2014/S9452/S9977/T1016/H0044+U5 - NOT A SELF-CONTAINED QUERY, A REMINDER TO WALTER TO DO THAT COMPARISON BEFORE THIS BECOMES THE STANDING DEFINITION
