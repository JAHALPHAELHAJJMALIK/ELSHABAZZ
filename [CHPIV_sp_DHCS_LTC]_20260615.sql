...
CREATE  OR ALTER PROCEDURE dbo.[CHPIV_sp_DHCS_LTC] -- <Procedure_Name, sysname, ProcedureName> ... DROP PROCEDURE ... EXEC dbo.[CHPIV_sp_DHCS_LTC]; 
...
-- =====================================================================
		...
		,@LineofBusiness AS varchar(255) = NULL -- '08/19/2015' -- DEFAULT VAL()
...
-- C001: ADD claimdrg 'MS-DRG (Medicare Severity Diagnosis Related Groups) CODES' TO DETAIL AS OF 20240523
		...
		INTO INFORMATICS.dbo.DELIVERABLE_DHCS_LTC_CHPIV -- ... Enter the number of members currently residing in LTC for >90 days during the reporting period
		FROM 
		( -- INITIATE ...
		SELECT aa.memid
		...
		FROM #AdmitAssessment AS aa
			...
			FROM INFORMATICS.dbo.DHCS_LTC_CHPIV AS developsource
...
-- C002: PER BRD / NARRATIVE "D-SNP Transmission of Admission Notification Narrative 04-01-2026 (002).docx"
		...
		ROOT CAUSE 1 - CATASTROPHIC HASH MATCH (2.07e15 ESTIMATED ROWS):
				The three-branch UNION in setclaim INNER JOINed claimdetail inside the
				CTE scope, then CLAIMDATA re-INNER JOINed claim again, then the final SELECT
				re-INNER JOINed claimdetail a third time. SQL Server's optimizer could not
				...
				targeted EXISTS / INNER JOIN predicates on revcode and billtype BEFORE
				touching the detail table at full scale. The UNION is eliminated.
				setclaim now INNER JOINs only #QUALIFYING_CLAIMS.
		...
		ROOT CAUSE 2 - CORRELATED IN SUBQUERIES ON claimdetail (~10 BILLION REBINDS):
				...
				FIX: Replaced with EXISTS(...) anti-INNER JOIN pattern and, at the
		...
		ROOT CAUSE 4 - NESTED LOOPS ON hcfaposlocation + contract (~1.29e10 REBINDS):
				...
				and selects Hash INNER JOIN or Merge INNER JOIN against a small build side. */
...
SET @when = NULL
SET @RangeStartDate = ISNULL(@StartDate,DATEADD(MONTH,@leadlag,DATEADD(DAY,1,EOMONTH(GETDATE(),-1)))) -- AS [1st OF ... MONTH]
SET @RangeEndDate = ISNULL(@EndDate,DATEADD(day,-1,DATEADD(MONTH,@leadlagmonths,@RangeStartDate)))
SET @CutoffDate =  TRY_CONVERT(date,GETDATE()) -- PAID DATE CUTOFF
SET @TruevFalse = NULL
-- SET @DYNAMICLIKE = '%'+ISNULL(@SearchString,'')+'%' -- LIKE '%[@PARAMETER]%'
SET @lobplanprog = ISNULL(@LineofBusiness,'MSO - CHPIV') -- SET @lobplanprog = 'DSNP MEDICARE PLAN' -- SELECT * FROM INFORMATICS.dbo.[uvw_LINE_OF_BUSINESS] -- ASSESS VALID ACTIVE OPTION(S)
...
DROP TABLE IF EXISTS INFORMATICS.dbo.DHCS_LTC_CHPIV;
DROP TABLE IF EXISTS INFORMATICS.dbo.DELIVERABLE_DHCS_LTC_CHPIV;
DROP TABLE IF EXISTS INFORMATICS.dbo.DELIVERABLE_DHCS_LTC_CHPIV_TEMPLATE;
...
WHERE 1=1
	AND cdrev.claimid = clm.claimid -- KEY ON ... 
	AND CASE 
...
-- =====================================================================
	--  SETCLAIM CTE: NOW INNER JOINS #QUALIFYING_CLAIMS ONLY - NOT FULL TABLES
-- =====================================================================
;WITH setclaim AS
( -- INITIATE ...
SELECT qc.claimid
...
INTO INFORMATICS.dbo.DHCS_LTC_CHPIV
-- SELECT TOP 10 ' ' AS 'CHECK 1st',*
-- SELECT DISTINCT ' ' AS 'CHECK 1st'
FROM CLAIMDATA
...
CREATE NONCLUSTERED INDEX idx_REFACTOR_DupID ON INFORMATICS.dbo.DHCS_LTC_CHPIV ([DupID]);
CREATE NONCLUSTERED INDEX idx_DSNP_claimid ON INFORMATICS.dbo.DHCS_LTC_CHPIV (claimid);
CREATE NONCLUSTERED INDEX idx_DSNP_memid ON INFORMATICS.dbo.DHCS_LTC_CHPIV (memid);

	-- CHECK FOR DUP(S) --
/* SELECT ' ' AS 'SHOULD RETURN 0 ROWS - DUP Validation',*
FROM INFORMATICS.dbo.DHCS_LTC_CHPIV
WHERE 1=1
    AND [DupID] IN
    ( -- INITIATE ...
    SELECT [DupID]
    FROM INFORMATICS.dbo.DHCS_LTC_CHPIV AS dup
    GROUP BY [DupID]
    HAVING COUNT(1)>1
    ) -- CONCLUDE ... */
...
-- =====================================================================
	--  THE FINAL CLAIM:
-- =====================================================================
UPDATE INFORMATICS.dbo.DHCS_LTC_CHPIV
SET [THE FINAL CLAIM] = 'N' -- POWER CYCLE RESET REFRESH RESTART ...

UPDATE INFORMATICS.dbo.DHCS_LTC_CHPIV
SET [THE FINAL CLAIM] = 'Y'
FROM INFORMATICS.dbo.DHCS_LTC_CHPIV AS clm
...
FROM INFORMATICS.dbo.DHCS_LTC_CHPIV AS c
...
FROM INFORMATICS.dbo.DHCS_LTC_CHPIV AS su
 ...
 FROM INFORMATICS.dbo.DHCS_LTC_CHPIV AS su 
	...
	FROM HMOPROD_PLANDATA.dbo.enrollkeys (NOLOCK) AS ek
		...
		INNER JOIN HMOPROD_PLANDATA.dbo.claim (NOLOCK) AS c ON ek.memid = c.memid
			AND ek.enrollid = c.enrollid -- planid / programid INNER JOIN
...
-- =====================================================================
UPDATE INFORMATICS.dbo.DHCS_LTC_CHPIV -- FIVE (5) COMPONENT(s) FIVE (5) table(s) - claim, claimdetail,provider, contractinfo, affiliation ... 
...
FROM INFORMATICS.dbo.DHCS_LTC_CHPIV AS source
	INNER JOIN 
	( -- INITIATE ...
	SELECT Cl.claimid
	,Cl.affiliationid
	...
	INNER JOIN HMOPROD_PLANDATA.dbo.contractinfo (NOLOCK) AS ci ON hdr.programid = ci.programid -- ci key#01 smh.programid,
		...
		AND TRY_CONVERT(date,source.[DOS]) BETWEEN TRY_CONVERT(date,ci.effdate) AND TRY_CONVERT(date,ci.termdate) -- ci key#04 ... NOT NEEDED WHEN INNER JOIN ON uvw_RELAXED_NETWORK_OON
...
FROM INFORMATICS.dbo.DHCS_LTC_CHPIV AS t 
...
FROM INFORMATICS.dbo.DHCS_LTC_CHPIV AS sourcetable
...
-----------------------------------------------------------------------------------------------------------------------------
	--Error Trapping: Check If Permanent Table(s) Already Exist(s) And Drop If Applicable--
-----------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS #AdmitAssessment -- SQL Server 2016 and later -- DYNAMIC tmp OR STATIC [TABLE] #INFORMATICS.dbo.DHCS_LTC_CHPIV is a local temporary table visible only IN the current session; ##INFORMATICS.dbo.DHCS_LTC_CHPIV is a GLOBAL temporary table visible to all sessions IN ('TempDB')
...
AS dothemath
		...
		FROM INFORMATICS.dbo.DHCS_LTC_CHPIV AS su -- UTILIZATION BETWEEN 2022-05-01 AND 2022-12-31
		WHERE 1=1 
			AND [Cx] IN (SELECT DISTINCT [Cx] FROM INFORMATICS.dbo.DHCS_LTC_CHPIV)
...
BEGIN
		...
		FROM #AdmitAssessment AS aa
			...
			FROM #AdmitAssessment 
			WHERE 1=1 
				AND [Index AdmitID] IS NOT NULL -- NO NOT NEGATIVE <> != ...
			) -- CONCLUDE ...
...
FROM #AdmitAssessment AS aa
	...
	SELECT DISTINCT c.affiliationid
	,c.memid
	,c.startdate
	,c.enddate
	,c.dischargedate
	...
	WHERE 1=1 
		AND c.claimid IN (SELECT DISTINCT claimid FROM INFORMATICS.dbo.DHCS_LTC_CHPIV)
		) AS t
		INNER JOIN HMOPROD_PLANDATA.dbo.affiliation (NOLOCK) AS a ON t.affiliationid = a.affiliationid -- REMINDER: within claim(s) tables AFFILIATIONID is the relationship between the a.provid = RENDERING PROVIDER AND the a.affiliateid = PAYTO aka VENDOR
	WHERE 1=1 
...
FROM #AdmitAssessment AS aa
	...
	WHERE 1=1
		...
		INNER JOIN HMOPROD_PLANDATA.dbo.affiliation AS a ON t.affiliationid = a.affiliationid -- REMINDER: within claim(s) tables AFFILIATIONID is the relationship between the a.provid = PROVIDER AND the a.affiliateid = PAYTO aka VENDOR
	WHERE 1=1 
		AND TRY_CONVERT(date,dc.calendar_date) BETWEEN TRY_CONVERT(date,t.startdate) AND TRY_CONVERT(date,ISNULL(t.Dischargedate,t.enddate)) -- RINA()
		-- AND TRY_CONVERT(date,dc.calendar_date) BETWEEN TRY_CONVERT(date,@CalendarDate_Start) AND TRY_CONVERT(date,@CalendarDate_End) -- RINA()
		-- AND claimid NOT LIKE '%A%'  -- see "STEP88_ORIGINAL_CLAIM...sql" ... 'A "CLEAN CLAIM" is one that has no defect, impropriety, lack of any required substantiating documentation, or particular circumstance requiring special treatment that prevents timely payment.'
...
FROM #AdmitAssessment AS uaa
	INNER JOIN 
	( -- INITIATE ...
	SELECT aa.[Index AdmitID]
	,COUNT(DISTINCT(dc.calendar_date))-1 AS [Index Calendar Days] -- EXCLUDE / minus THE END / DISCHARGE date
	-- SELECT TOP 10 ' ' AS 'CHECK 1st',*
	-- SELECT DISTINCT ' ' AS 'CHECK 1st'
	FROM INFORMATICS.dbo.date_calendarISO AS dc
	CROSS APPLY #AdmitAssessment AS aa -- INDEPENDENT STAND ALONE ... CROSS APPLY() CROSS JOIN() CARTESIAN PRODUCT
	WHERE 1=1 
...
FROM #AdmitAssessment AS aa
	-- INNER JOIN INFORMATICS.dbo.DHCS_LTC_CHPIV AS developsource ON aa.AdmitID = developsource.AdmitID
WHERE 1=1 
...
WHERE 1=1
	AND los.[DAY(S)] >= @ltcdays -- ... Enter the number of members currently residing in LTC for >90 days during the reporting period
		...
		--  SELECT * FROM INFORMATICS.dbo.DELIVERABLE_DHCS_LTC_CHPIV
...
-- ,s.*,d.*
FROM INFORMATICS.dbo.DELIVERABLE_DHCS_LTC_CHPIV AS s
	INNER JOIN INFORMATICS.dbo.DHCS_LTC_CHPIV AS d ON s.memid = d.memid
WHERE 1=1
...
INTO INFORMATICS.dbo.DELIVERABLE_DHCS_LTC_CHPIV_TEMPLATE
FROM INFORMATICS.dbo.DELIVERABLE_DHCS_LTC_CHPIV
CROSS APPLY (SELECT DISTINCT [RANGE NOTE(S)] FROM INFORMATICS.dbo.DHCS_LTC_CHPIV) AS rp -- INDEPENDENT STAND ALONE ... CROSS APPLY() CROSS INNER JOIN() CARTESIAN PRODUCT
WHERE 1=1
GROUP BY rp.[RANGE NOTE(S)]

		SELECT * FROM INFORMATICS.dbo.DELIVERABLE_DHCS_LTC_CHPIV_TEMPLATE
-- =====================================================================
