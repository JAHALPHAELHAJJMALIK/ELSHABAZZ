...
DROP TABLE IF EXISTS INFORMATICS.dbo.CONTRACTING_LOHAR;
DROP TABLE IF EXISTS #ContractAnalysis;
DROP TABLE IF EXISTS INFORMATICS.dbo.CONTRACTING_LOHAR_TEMPLATE_DETAIL;
...
WHERE 1=1 
	AND -- C008: LO-HAR SENIOR LIVING -- TIN DISCREPANCY: eVIPs TIN 471814132 vs. request doc TIN 471614132 -- BOTH retained per Sir Salim confirmation email 20260612 | NPI: 1104229012


	( -- INITIATE ...
	UPPER(LTRIM(RTRIM(ISNULL(prov.fedid,'')))) IN ('471814132','471614132') -- TIN (TAXPAYER IDENTIFICATION NUMBER) aka TaxID
		OR UPPER(LTRIM(RTRIM(ISNULL(prov.NPI,'')))) IN ('1104229012')  -- NPI (National Provider ID)
...
INTO INFORMATICS.dbo.CONTRACTING_LOHAR
-- SELECT TOP 10 ' ' AS 'CHECK 1st',*
-- SELECT DISTINCT ' ' AS 'CHECK 1st'
FROM CLAIMDATA
...
CREATE NONCLUSTERED INDEX idx_REFACTOR_DupID ON INFORMATICS.dbo.CONTRACTING_LOHAR ([DupID]);
CREATE NONCLUSTERED INDEX idx_DSNP_claimid ON INFORMATICS.dbo.CONTRACTING_LOHAR (claimid);
CREATE NONCLUSTERED INDEX idx_DSNP_memid ON INFORMATICS.dbo.CONTRACTING_LOHAR (memid);

	-- CHECK FOR DUP(S) --
SELECT ' ' AS 'SHOULD RETURN 0 ROWS - DUP Validation',*
FROM INFORMATICS.dbo.CONTRACTING_LOHAR
WHERE 1=1
    AND [DupID] IN
    ( -- INITIATE ...
    SELECT [DupID]
    FROM INFORMATICS.dbo.CONTRACTING_LOHAR AS dup
    GROUP BY [DupID]
    HAVING COUNT(1)>1
    ) -- CONCLUDE ...
...
-- =====================================================================
	--  THE FINAL CLAIM:
-- =====================================================================
UPDATE INFORMATICS.dbo.CONTRACTING_LOHAR
SET [THE FINAL CLAIM] = 'N' -- POWER CYCLE RESET REFRESH RESTART ...

UPDATE INFORMATICS.dbo.CONTRACTING_LOHAR
SET [THE FINAL CLAIM] = 'Y'
FROM INFORMATICS.dbo.CONTRACTING_LOHAR AS clm
...
FROM INFORMATICS.dbo.CONTRACTING_LOHAR AS c
...
FROM INFORMATICS.dbo.CONTRACTING_LOHAR AS su
...
-- =====================================================================
UPDATE INFORMATICS.dbo.CONTRACTING_LOHAR -- FIVE (5) COMPONENT(s) FIVE (5) table(s) - claim, claimdetail,provider, contractinfo, affiliation ... 
...
FROM INFORMATICS.dbo.CONTRACTING_LOHAR AS source
	INNER JOIN 
	( -- INITIATE ...
	SELECT Cl.claimid
	,Cl.affiliationid
...
FROM INFORMATICS.dbo.CONTRACTING_LOHAR AS t 
...
FROM INFORMATICS.dbo.CONTRACTING_LOHAR AS sourcetable
...
FROM INFORMATICS.dbo.CONTRACTING_LOHAR AS u
...
-- M001: Replace hardcoded @SETfeesched = 'C01350904' with a DYNAMIC
      ...
      -- sourced directly from INFORMATICS.dbo.CONTRACTING_LOHAR.
      ...
      └─────────────────────────────────┴─────────────────┴───────────────┘ */
		...
		formtype columns already staged in CONTRACTING_LOHAR.
...
--    columns already present in INFORMATICS.dbo.CONTRACTING_LOHAR.
--
...
* (CASE WHEN md5.paypercent IS NULL THEN 1 ELSE md5.paypercent/100 END)
* su.QUANTITY
FROM INFORMATICS.dbo.CONTRACTING_LOHAR AS su   (NOLOCK)
...
-- YEAR DISCRIMINATOR: [YEAR_DOS] (yyyymm substring already staged as 4-char year in CONTRACTING_LOHAR)
...
FROM INFORMATICS.dbo.CONTRACTING_LOHAR AS er
WHERE 1=1
	AND CASE
...
FROM #ContractAnalysis AS ca
		...
		-- SELECT QNXTbilltype,BillTypeDescr FROM INFORMATICS.dbo.CONTRACTING_LOHAR GROUP BY QNXTbilltype,BillTypeDescr
...
INTO INFORMATICS.dbo.CONTRACTING_LOHAR_TEMPLATE_DETAIL
-- SELECT TOP 10 ' ' AS 'CHECK 1st',*
-- SELECT DISTINCT ' ' AS 'CHECK 1st'
FROM INFORMATICS.dbo.CONTRACTING_LOHAR AS su
	INNER JOIN #ContractAnalysis AS ca ON su.memid = ca.memid
		...
		-- SELECT * FROM INFORMATICS.dbo.CONTRACTING_LOHAR_TEMPLATE_DETAIL
...
FROM [QNXT_Custom].[TRI].[FfsPaymentDetails] AS tri
	INNER JOIN INFORMATICS.dbo.CONTRACTING_LOHAR AS c ON tri.ClaimId = c.claimid
...
GROUP BY tri.PaytoProvId,piso.PROVNM
		...
		,[TRI Payment],*
		FROM INFORMATICS.dbo.CONTRACTING_LOHAR_TEMPLATE_DETAIL
		WHERE 1=1
			AND ISNULL(claimid,'') IN ('25213E75618','25071E12217') -- C008: REPLACE WITH LO-HAR SAMPLE CLAIMID(S) FOR QA
