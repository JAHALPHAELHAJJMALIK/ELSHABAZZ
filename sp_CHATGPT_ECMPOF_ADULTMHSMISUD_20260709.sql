...
--/****** Object:  StoredProcedure [dbo].[sp_CHATGPT_ECMPOF_ADULTMHSMISUD]    Script Date: 6/29/2026 4:15:42 PM ******/
--SET ANSI_NULLS ON
--GO
...
-- CREATE OR ALTER PROCEDURE [dbo].[sp_CHATGPT_ECMPOF_ADULTMHSMISUD] -- <Procedure_Name, sysname, ProcedureName> ... DROP PROCEDURE ... EXEC dbo.sp_CHATGPT_ECMPOF_ADULTMHSMISUD;
...
DECLARE @PCPrecordRANK AS int = 1 -- ENTER 1 TO GET() JUST MOST CURRENT [RANKis] see DENSE_RANK() OVER(PARTITION BY ek.memid ORDER BY CAST(ek.effdate AS datetime) DESC) AS [RANKis] --'RANKis' --RANK by MOST CURRENT
		...
		SELECT TOP 1 @AgeCutoff AS [AGE LIMIT >= (GREATER THAN EQUAL TO)],@ReportRunDate AS [RunDate],'BETWEEN '+CAST(CAST(@StartDate AS date) AS varchar(MAX))+' AND '+CAST(CAST(@EndDate AS date) AS varchar(MAX)) AS [RANGE NOTE(s)],*
...
--------------------------------------------------------------------------------------------------------------------
	-- Error Trapping: Check If Permanent Table(s) Already Exist(s) And Drop If Applicable --
--------------------------------------------------------------------------------------------------------------------
-- TRUNCATE TABLE INFORMATICS.dbo.CHATGPT_ECMPOF_ADULT_MHSMISUD; -- SQL Server 2016 and later -- DYNAMIC tmp OR STATIC [TABLE] #TableName is a local temporary table visible only in the current session; ##TableName is a GLOBAL temporary table visible to all sessions IN ('TempDB')

-- DROP TABLE IF EXISTS INFORMATICS.dbo.CHATGPT_ECMPOF_ADULT_MHSMISUD; -- SQL Server 2016 and later -- DYNAMIC tmp OR STATIC [TABLE] #TableName is a local temporary table visible only in the current session; ##TableName is a GLOBAL temporary table visible to all sessions IN ('TempDB')

INSERT INTO INFORMATICS.dbo.CHATGPT_ECMPOF_ADULT_MHSMISUD -- (FIELD(S) IN PLAY,...)

SELECT finalinsert.*
FROM 
( -- INITIATE ...
SELECT DISTINCT 'Adult SMH or SUD' AS [TargetPopulation],@AgeCutoff AS [AGE LIMIT >= (GREATER THAN EQUAL TO)],@ReportRunDate AS [RunDate],'BETWEEN '+CAST(CAST(@StartDate AS date) AS varchar(MAX))+' AND '+CAST(CAST(@EndDate AS date) AS varchar(MAX)) AS [RANGE NOTE(s)]
...
-- INTO INFORMATICS.dbo.CHATGPT_ECMPOF_ADULT_MHSMISUD
-- SELECT TOP 10 ' ' AS 'CHECK 1st',*
-- SELECT DISTINCT ' ' AS 'CHECK 1st',
...
WHERE 1=1
	...
	-- Display results
		-- SELECT * FROM INFORMATICS.dbo.CHATGPT_ECMPOF_ADULT_MHSMISUD

		SELECT 'ECM POF Adult SMH or SUD' AS ' '
