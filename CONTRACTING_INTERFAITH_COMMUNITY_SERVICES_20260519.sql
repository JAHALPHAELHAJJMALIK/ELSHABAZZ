...
DROP TABLE IF EXISTS INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES;
DROP TABLE IF EXISTS #ContractAnalysis;
DROP TABLE IF EXISTS INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES_TEMPLATE_DETAIL;
...
WHERE 1=1 
	AND -- Subject: Rate Increase Request -- Interfaith Community Services -- TIN: 953837714 NPI: 1538667969
	( -- INITIATE ...
	UPPER(LTRIM(RTRIM(ISNULL(prov.fedid,'')))) IN ('953837714') -- TIN (TAXPAYER IDENTIFICATION NUMBER) aka TaxID
		OR UPPER(LTRIM(RTRIM(ISNULL(prov.NPI,'')))) IN ('1538667969')  -- NPI (National Provider ID)
		) -- CONCLUDE ...
...
THEN 'CONTRACTING RENDERING PROVIDER'
WHEN EXISTS ( -- DEFINITIVE ANSWER: Returns TRUE the moment SQL Server finds EVEN ONE MATCHING ROW IN THE SUBQUERY. It immediately stops scanning -- it does not count rows, does not return data, just confirms presence. This is why SELECT 1 is used -- the actual column value is irrelevant. -- IN PLAIN LANGUAGE: "DOES AT LEAST ONE ROW EXIST IN #PROVISOLATION where the provider ID matches?" ... NOT EXISTS(): EXACT LOGICAL INVERSE. Returns TRUE only WHEN THE SUBQUERY FINDS ZERO MATCHING ROWS. It is the correct, sargable, SQL-idiomatic way to express "exclude any claim whose provider appears in this list." It is semantically equivalent to a LEFT JOIN ... WHERE joined_table.key IS NULL anti-join pattern, but often cleaner and more readable.
SELECT 1
FROM #PROVISOLATION AS piso
WHERE 1=1
...
THEN 'CONTRACTING PAYTO PROVIDER'
ELSE 'NOT_QUALIFYING'
...
WHERE 1=1
    ...
    HAVING COUNT(1)>1
        ...
        SELECT ' ' AS 'QA PRE-STAGE - QUALIFYING CLAIM COUNT: '
...
INTO INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES
-- SELECT TOP 10 ' ' AS 'CHECK 1st',*
-- SELECT DISTINCT ' ' AS 'CHECK 1st'
FROM CLAIMDATA
...
CREATE NONCLUSTERED INDEX idx_REFACTOR_DupID ON INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES ([DupID]);
CREATE NONCLUSTERED INDEX idx_DSNP_claimid ON INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES (claimid);
CREATE NONCLUSTERED INDEX idx_DSNP_memid ON INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES (memid);

	-- CHECK FOR DUP(S) --
SELECT ' ' AS 'SHOULD RETURN 0 ROWS - DUP Validation',*
FROM INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES
WHERE 1=1
    AND [DupID] IN
    ( -- INITIATE ...
    SELECT [DupID]
    FROM INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES AS dup
    GROUP BY [DupID]
    HAVING COUNT(1)>1
    ) -- CONCLUDE ...
...
-- =====================================================================
	--  THE FINAL CLAIM:
-- =====================================================================
UPDATE INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES
SET [THE FINAL CLAIM] = 'N' -- POWER CYCLE RESET REFRESH RESTART ...

UPDATE INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES
SET [THE FINAL CLAIM] = 'Y'
FROM INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES AS clm
...
FROM INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES AS c
...
FROM INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES AS su
...
-- =====================================================================
UPDATE INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES -- FIVE (5) COMPONENT(s) FIVE (5) table(s) - claim, claimdetail,provider, contractinfo, affiliation ... 
...
FROM INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES AS source
	INNER JOIN 
	( -- INITIATE ...
	SELECT Cl.claimid
	,Cl.affiliationid
...
FROM INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES AS t 
...
FROM INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES AS sourcetable
...
FROM INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES AS u
...
-- M001: Replace hardcoded @SETfeesched = 'C01350904' with a DYNAMIC
      ...
      -- sourced directly from INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES.
      ...
      └─────────────────────────────────┴─────────────────┴───────────────┘ */
		...
		formtype columns already staged in CONTRACTING_INTERFAITH_COMMUNITY_SERVICES.
...
--    columns already present in INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES.
--
...
* (CASE WHEN md5.paypercent IS NULL THEN 1 ELSE md5.paypercent/100 END)
* su.QUANTITY
FROM INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES AS su   (NOLOCK)
...
,CASE -- C005: INTERFAITH COMMUNITY SERVICES 20260519 -- REPLACES ELLNER BARIATRIC CASE BLOCK -- Medi-Cal LOB only -- Per Diem T2033-U6 basis -- Per Daphyne Watson contracting request 20260511
WHEN UPPER(LTRIM(RTRIM(ISNULL([CPT Service Code], '')))) IN ('T2033') -- Recuperative Care (T2033-U6 $204/day current -> $225/day proposed)
    AND 
	( -- INITIATE ...
	modcode = 'U6'
		OR modcode2 = 'U6'
...
ELSE CAST('Other Services' AS nvarchar(255)) -- services outside Recuperative Care Medi-Cal scope -- default catch-all
END AS 'Contracting_Proposal_Category'
,CASE -- C005: INTERFAITH COMMUNITY SERVICES 20260519 -- REPLACES ELLNER BARIATRIC CASE BLOCK -- Per Diem T2033-U6 $204/day -> $225/day -- Medi-Cal LOB only
WHEN UPPER(LTRIM(RTRIM(ISNULL([CPT Service Code], '')))) IN ('T2033') -- Recuperative Care T2033-U6
    AND 
	( -- INITIATE ...
	modcode = 'U6'
		OR modcode2 = 'U6'
...
ELSE 0 -- DEFAULT: specialty provider -- uncategorized lines are likely data anomaly -- hard stop per corpus standard
...
FROM INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES AS er
WHERE 1=1
	AND CASE
...
FROM #ContractAnalysis AS ca
		...
		-- SELECT QNXTbilltype,BillTypeDescr FROM INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES GROUP BY QNXTbilltype,BillTypeDescr
...
INTO INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES_TEMPLATE_DETAIL
-- SELECT TOP 10 ' ' AS 'CHECK 1st',*
-- SELECT DISTINCT ' ' AS 'CHECK 1st'
FROM INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES AS su
	INNER JOIN #ContractAnalysis AS ca ON su.memid = ca.memid
		...
		-- SELECT * FROM INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES_TEMPLATE_DETAIL
...
WHERE 1=1
	...
	END IN ('169','190') -- LIMIT PLACED ON REVENUE_CODE
	) --CONCLUDE ...
...
FROM [QNXT_Custom].[TRI].[FfsPaymentDetails] AS tri
	INNER JOIN [INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES] AS c ON tri.ClaimId = c.claimid
...
GROUP BY tri.PaytoProvId,piso.PROVNM
		...
		FROM INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES
		WHERE 1=1
			AND ISNULL(claimid,'') IN ('25213E75618','25071E12217')
		...
		,[TRI Payment],*
		FROM INFORMATICS.dbo.CONTRACTING_INTERFAITH_COMMUNITY_SERVICES_TEMPLATE_DETAIL
		WHERE 1=1
			AND ISNULL(claimid,'') IN ('25213E75618','25071E12217')
