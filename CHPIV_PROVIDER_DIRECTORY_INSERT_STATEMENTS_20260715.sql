...
-- C004: AD HOC MANUAL ADD-ON — FORCE 2 HOSPITAL RECORDS into CHPIV_PROVIDER_DIRECTORY_DATA

		/* SOURCE FILE:
			- "FORCE CHPIV HOSPITALS Provider Directory Data to CHG 12_08_25.xlsx" -> worksheet "2 HOSPITALS" (2 rows)

		DESTINATION TABLE:
			- CHPIV_PROVIDER_DIRECTORY_DATA

		NOTES:
			- No staging table exists for these 2 records; values are hard-coded (literal VALUES) rather than SELECT'd from an UPDATE table
			- Records: El Centro Regional Medical Center (NPI 1861409823) and Pioneers Memorial Hospital (NPI 1073519443)
			- COUNTY forced to 'Imperial' (business rule, consistent with C001/C002)
			- Telehealth_Indicator, [TAXONOMY CODE], [TRAINING], [LOCATION NPI], [LOCATION NAME], [CULTURAL COMPETENCY], [HCAI ID] not present in source -> NULL
			- [STAFFED BEDS] not present in source -> defaulted to 0, consistent with C002/C003 pattern
			- ACCESS_REQ present in source ('Basic' for both) but excluded from destination column list, consistent with existing INSERT (C002) */
...
BEGIN TRY
    ...
    INSERT INTO INFORMATICS.dbo.CHPIV_PROVIDER_DIRECTORY_DATA
    ( -- INITIATE ...
...
-- =====================================================================================
	-- C004: AD HOC MANUAL ADD-ON — FORCE 2 HOSPITAL RECORDS - SOURCE: "FORCE CHPIV HOSPITALS Provider Directory Data to CHG 12_08_25.xlsx" -> "2 HOSPITALS"
-- =====================================================================================

-- =====================================================
-- SECTION 1: PRE-INSERT VALIDATION (AD HOC)
-- =====================================================
DECLARE @Main_Count_Before_C004 INT;
DECLARE @ImportedAt_C004 DATETIME2 = SYSDATETIME();
DECLARE @ImportedBy_C004 NVARCHAR(128) = SYSTEM_USER;

SELECT @Main_Count_Before_C004 = COUNT(*)
FROM INFORMATICS.dbo.CHPIV_PROVIDER_DIRECTORY_DATA;

PRINT 'C004 AD HOC ADD-ON: FORCE 2 HOSPITAL RECORDS';
PRINT '  Main Table Records (Before): ' + CAST(@Main_Count_Before_C004 AS VARCHAR(10));
PRINT '  Expected Records After Insert: ' + CAST((@Main_Count_Before_C004 + 2) AS VARCHAR(10));
PRINT '';

-- =====================================================
-- SECTION 2: INSERT (LITERAL VALUES — NO STAGING TABLE)
-- =====================================================
BEGIN TRY
    BEGIN TRANSACTION;
	...
	-- Language Information (25 columns — not present in source, NULL)
	,PHY_LANG01_PRV_LANG01
	,PHY_LANG02_PRV_LANG02
	,PHY_LANG03_PRV_LANG03
	,PHY_LANG04_PRV_LANG04
	...
	-- Extender Information (16 columns — not present in source, NULL)
	,EXT1A_LNAME
	,EXT1B_FNAME
	,EXT1C_MINITIAL
	,EXT1D_LICTYPE
	...
	-- Accessibility Indicators (8 columns — ACCESS_REQ excluded, consistent with existing INSERT)
	-- ,ACCESS_REQ
	,PARKING_IND
	,EXT_BUILD_IND
	,INT_BUILD_IND
	...
	-- Additional Provider Information (3 columns)
	,LIC_ID
	,EMAIL
	,BOARD_CERT_ID

	-- Office Language Information (15 columns — not present in source, NULL)
	,OFFICE_LANG01
	,OFFICE_LANG02
	,OFFICE_LANG03
	,OFFICE_LANG04
	...
	-- C003: Network Analysis ADD-ON COLUMN(S) — not present in source, NULL/0 defaulted
	,[TAXONOMY CODE]
	,[TRAINING]
	,[LOCATION NPI]
	,[LOCATION NAME]
	...
	VALUES
	-- Record 1: El Centro Regional Medical Center
	(
		'El Centro','CHPIV Direct ','Acute Care Hospital','El Centro Regional Medical Center',NULL,NULL,NULL,NULL
		,'1415 Ross Ave','CA',92243,7603397100,'24 HRS'
		,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL
		,NULL,NULL
		,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL
		,3,'Imperial',NULL,'Direct',NULL,NULL,'1861409823'
		,'P','EB','IB','R','E','T','PA','PD'
		,106130699,NULL,'N'
		,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL
		,NULL,NULL,NULL,NULL,'https://www.ecrmc.org/',NULL
		,NULL
		,NULL,NULL,'TJC','The Joint Commission'
		,NULL
		,@ImportedAt_C004,@ImportedBy_C004
		,'Imperial',NULL
		,NULL,NULL,NULL,NULL,0,NULL,NULL
	),
	-- Record 2: Pioneers Memorial Hospital
	(
		'Brawley','CHPIV Direct ','Acute Care Hospital','Pioneers Memorial Hospital',NULL,NULL,NULL,NULL
		,'207 W D St','CA',92227,7603513333,'24 HRS'
		,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL
		,NULL,NULL
		,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL
		,3,'Imperial',NULL,'Direct',NULL,NULL,'1073519443'
		,'P','EB','IB','R','E','T','PA','PD'
		,90000087,NULL,'N'
		,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL
		,NULL,NULL,NULL,NULL,'https://pmhd.org/',NULL
		,NULL
		,NULL,NULL,'DNV','Healthcare Inc DNV'
		,NULL
		,@ImportedAt_C004,@ImportedBy_C004
		,'Imperial',NULL
		,NULL,NULL,NULL,NULL,0,NULL,NULL
    ...
    DECLARE @Inserted_C004 INT = @@ROWCOUNT;

    COMMIT TRANSACTION;

    PRINT 'C004 INSERT COMPLETED SUCCESSFULLY';
    PRINT '  Records Inserted: ' + CAST(@Inserted_C004 AS VARCHAR(10));
    PRINT '';
...
BEGIN CATCH
    ...
    PRINT 'ERROR INSERTING C004 RECORDS:';
    PRINT '  Error Number: ' + CAST(ERROR_NUMBER() AS VARCHAR(10));
    PRINT '  Error Message: ' + ERROR_MESSAGE();
    PRINT '  Error Line: ' + CAST(ERROR_LINE() AS VARCHAR(10));
    PRINT '';
...
-- =====================================================
-- SECTION 3: POST-INSERT VALIDATION (AD HOC)
-- =====================================================
DECLARE @Main_Count_After_C004 INT;

SELECT @Main_Count_After_C004 = COUNT(*)
FROM INFORMATICS.dbo.CHPIV_PROVIDER_DIRECTORY_DATA;

PRINT 'C004 Final Record Counts:';
PRINT '  Main Table Records (After): ' + CAST(@Main_Count_After_C004 AS VARCHAR(10));
PRINT '  Total Records Added: ' + CAST((@Main_Count_After_C004 - @Main_Count_Before_C004) AS VARCHAR(10));
PRINT '';

-- Verify the 2 forced hospital records landed
PRINT 'C004 Verification — forced hospital records:';
SELECT LNAME,CITY,NPI_NO,COUNTY,PPG_NAME,ImportedAt,ImportedBy
FROM INFORMATICS.dbo.CHPIV_PROVIDER_DIRECTORY_DATA
WHERE 1=1
	AND NPI_NO IN ('1861409823','1073519443')
ORDER BY LNAME;

PRINT '';
PRINT '  Expected: 2 rows above (El Centro Regional Medical Center, Pioneers Memorial Hospital)';
PRINT '  If 0 rows: check @ImportedAt_C004 datetime2 precision against column type/collation';
PRINT '  If >2 rows: @ImportedAt_C004 collided with another batch''s SYSDATETIME() value (extremely unlikely)';
-- =================================================================
-- C004 AD HOC ADD-ON COMPLETED
-- =================================================================
GO
