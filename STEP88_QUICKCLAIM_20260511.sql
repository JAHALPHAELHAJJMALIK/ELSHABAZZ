...
SET @BirthdayAgeParam = DATEADD( yy, -18, GETDATE()) -- WHERE 1=1 AND TRY_CONVERT(date,mem.dob) <= @BirthdayAgeParam -- 65+ MEMBERSHIP
		...
		,DATEFROMPARTS(YEAR(DATEADD(MONTH,1,GETDATE())),MONTH(DATEADD(MONTH,1,GETDATE())),1) AS 'NormalizeNextMonth',bp.*
...
WHEN sbt.QNXTbilltype LIKE '1[1-2]%'
THEN 'HOSPITAL_INPATIENT'
WHEN sbt.QNXTbilltype LIKE '2[1-3]%'
THEN 'SNF_SKILLED'
...
WHERE 1=1
	AND cdx.claimid = clm.claimid -- KEY ON ... 
	AND CASE
	WHEN SUBSTRING(UPPER(LTRIM(RTRIM(ISNULL(cdx.revcode,'')))),1,1) = '0'
	THEN SUBSTRING(UPPER(LTRIM(RTRIM(ISNULL(cdx.revcode,'')))),2,3)
	ELSE UPPER(LTRIM(RTRIM(ISNULL(cdx.revcode,''))))
...
WHERE 1=1
	...
	AND SUBSTRING(UPPER(LTRIM(RTRIM(ISNULL(cdx.servcode,'')))),1,5) IN ('43644') -- Bariatric Surgery / Gastrectomy (Longitudinal Gastrectomy removes portion of stomach)
	)
