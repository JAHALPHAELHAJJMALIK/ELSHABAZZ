...
SELECT ' ' AS 'BASELINE SUMMARY BY LOB',GETDATE() AS [TimeStamp]
,LINE_OF_BUSINESS
,[RANGE NOTE(s)]
...
,SUM([QUANTITY]) AS [Qty]
,SUM([HCT LOGIC DAY(S)]) AS [HCT DAYS]
...
SELECT ' ' AS 'BASELINE COUNTERPROPOSAL SUMMARY',[TimeStamp]
,LINE_OF_BUSINESS
,[RANGE NOTE(s)]
,COUNT(DISTINCT(memid)) AS 'Unique Members'
...
SELECT ' ' AS 'BASELINE PRIMARY LIMITED REVCODE COUNTERPROPOSAL SUMMARY',[TimeStamp]
,LINE_OF_BUSINESS
,[RANGE NOTE(s)]
,COUNT(DISTINCT(memid)) AS 'Unique Members'
,COUNT(DISTINCT(claimid)) AS [Unique Claims]
