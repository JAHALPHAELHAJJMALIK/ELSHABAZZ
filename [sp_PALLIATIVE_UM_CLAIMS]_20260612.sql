...
CREATE  OR ALTER PROCEDURE dbo.[sp_PALLIATIVE_UM_CLAIMS] -- <Procedure_Name, sysname, ProcedureName> ... DROP PROCEDURE ... EXEC dbo.[sp_PALLIATIVE_UM_CLAIMS]; 
...
SET @lobplanprog = ISNULL(@LineofBusiness,'DSNP') -- bp.[OPTION(S)]: IN ('COMMUNITY Y MAS (HMO C-SNP)','DSNP MEDI-CAL PLAN','MEDI-CAL BENEFIT PLAN') ... p.[OPTION(S)]: IN ('DSNP MEDI-CAL','DSNP MEDICARE','COMMUNITY Y MAS (HMO C-SNP)','MEDI-CAL') -- ACTIVE LOB LOCK ... 
...
DROP TABLE IF EXISTS INFORMATICS.dbo.QA_PALLIATIVE_CLAIM;
DROP TABLE IF EXISTS #PREDISQUALIFYING_AUTHS; -- PRE - FILTER EXCLUSION
DROP TABLE IF EXISTS #QUALIFYING_AUTHS; -- FILTER THE DETAIL
DROP TABLE IF EXISTS INFORMATICS.dbo.QA_PALLIATIVE_AUTH;
DROP TABLE IF EXISTS INFORMATICS.dbo.TEMPLATE_PALLIATIVE;
...
WHERE 1=1
	AND clm.claimid = cdx.claimid -- KEY ON ... 
...
DELETE -- ENSURE EXCLUSION FENCE ... now only blocks members who were confirmed palliative before the reporting period ...
...
FROM HMOPROD_PLANDATA.dbo.claim (NOLOCK) AS clm
	...
	-- INNER JOIN HMOPROD_PLANDATA.dbo.affiliation (NOLOCK) AS a ON clm.affiliationid = a.affiliationid -- REMINDER: within claim(s) tables AFFILIATIONID is the relationship BETWEEN the a.provid = [RENDERING] PROVIDER AND the a.affiliateid = [PAYTO] PROVIDER	
WHERE 1=1
	/* AND NOT TRY_CONVERT(date,cd.dosfrom) < @RangeStartDate -- NO NOT NEGATIVE <> != ... A snowflake sp CRITERIA "PROD_CHG_ODS.INFORMATICS.sp_PALLIATIVE_UM_CLAIMSDSNP_PALLIATIVE_QUARTERLY" */
	AND bp.[LINE_OF_BUSINESS] LIKE ISNULL('%'+@lobplanprog+'%','%') -- CARDINALITY HIERARCHY CASUALTY ... LEAVE AS FINAL FILTER
	AND bp.[BRAND] IN (ISNULL(@BRAND,'CHGSD')) -- ('MEDI-CAL BENEFIT PLAN','DSNP MEDI-CAL PLAN','COMMUNITY Y MAS (HMO C-SNP)','IV_DSNP2 
...
SET @Authlob =  'DSNP' -- bp.[OPTION(S)]: IN ('COMMUNITY Y MAS (HMO C-SNP)','DSNP MEDI-CAL PLAN','MEDI-CAL BENEFIT PLAN') ... p.[OPTION(S)]: IN ('DSNP MEDI-CAL','DSNP MEDICARE','COMMUNITY Y MAS (HMO C-SNP)','MEDI-CAL') -- ACTIVE LOB LOCK ... 
-- SET @PalliativeDx ='Z51.5'
...
SELECT DISTINCT ' ' AS 'DISQUALIFYING AUTH ANCHOR DATE: '
,prev_r.memid
,prev_r.referralid
,prev_r.authorizationid
,CASE
WHEN EXISTS ( -- DEFINITIVE ANSWER: Returns TRUE the moment SQL Server finds EVEN ONE MATCHING ROW IN THE SUBQUERY. It immediately stops scanning -- it does not count rows, does not return data, just confirms presence. This is why SELECT 1 is used -- the actual column value is irrelevant. -- IN PLAIN LANGUAGE: "DOES AT LEAST ONE ROW EXIST IN #PROVISOLATION where the provider ID matches?" ... NOT EXISTS(): EXACT LOGICAL INVERSE. Returns TRUE only WHEN THE SUBQUERY FINDS ZERO MATCHING ROWS. It is the correct, sargable, SQL-idiomatic way to express "exclude any claim whose provider appears in this list." It is semantically equivalent to a LEFT JOIN ... WHERE INNER JOINed_table.key IS NULL anti-INNER JOIN pattern, but often cleaner and more readable. ...SCENARIO B: REVCODE DRIVES CLASSIFICATION WHEN NO BILLTYPE MATCH ... C002: PER BRD / NARRATIVE "D-SNP Transmission of Admission Notification Narrative 04-01-2026 (002).docx"
SELECT 1
FROM HMOPROD_PLANDATA.dbo.authdiag (NOLOCK) AS adx
WHERE 1=1
	AND prev_r.referralid = adx.referralid -- KEY ON ... 
	AND adx.diagcode LIKE ISNULL('%'+@PalliativeDx+'%','%')
	)
THEN 'PALLIATIVE'
ELSE 'NOT_QUALIFYING'
END AS [InclusionExclusionType]
INTO #PREDISQUALIFYING_AUTHS
FROM HMOPROD_PLANDATA.dbo.referral (NOLOCK) AS prev_r
	INNER JOIN HMOPROD_PLANDATA.dbo.enrollkeys (NOLOCK) AS prev_e ON prev_r.enrollid = prev_e.enrollid
		AND prev_r.effdate BETWEEN CAST(prev_e.effdate AS date) AND CAST(ISNULL(prev_e.termdate,'12/31/2078') AS date) -- RINA: APPLIED TO FIELD WITHIN TABLE TO BE UPDATE	
	INNER JOIN INFORMATICS.dbo.[uvw_LINE_OF_BUSINESS] (NOLOCK) AS prev_bp ON prev_e.programid = prev_bp.programid -- HMOPROD_PLANDATA.dbo.benefitplan + HMOPROD_PLANDATA.dbo.program	
WHERE 1=1
	AND prev_bp.[LINE_OF_BUSINESS] LIKE ISNULL('%'+@Authlob+'%','%')
	AND prev_bp.[BRAND] IN (ISNULL(@BRAND,'CHGSD'))
	AND TRY_CONVERT(date, prev_r.seendate) < @ClockStart 

CREATE CLUSTERED INDEX idx_QAR_referralidclaimid ON #PREDISQUALIFYING_AUTHS (referralid);
CREATE NONCLUSTERED INDEX idx_QAR_memid ON #PREDISQUALIFYING_AUTHS (memid);

DELETE -- ENSURE EXCLUSION FENCE ... now only blocks members who were confirmed palliative before the reporting period ...
FROM #PREDISQUALIFYING_AUTHS
WHERE 1=1
	AND InclusionExclusionType = 'NOT_QUALIFYING'
...
WHERE 1=1
    ...
    ( -- INITIATE ...
    SELECT [referralid]
    FROM #PREDISQUALIFYING_AUTHS AS dup
    GROUP BY [referralid]
    HAVING COUNT(1)>1
    ) -- CONCLUDE ...

        SELECT ' ' AS 'QA PRE-STAGE OF DISQUALIFYING UM AUTH REFERRAL COUNT: '
        ,InclusionExclusionType
...
WHERE 1=1
	...
	WHERE 1=1
		AND NOT InclusionExclusionType = 'NOT_QUALIFYING' -- NO NOT NEGATIVE <> = ...
	GROUP BY memid -- TO ENSURE CAPTURE OF '... members NEWLY enrolled in palliative care services within the reporting period.'
	) -- CONCLUDE ... 
...
INTO INFORMATICS.dbo.QA_PALLIATIVE_AUTH
FROM SETAUTHREF;

		/* SELECT ' ' AS 'Auth - Palliative Members IN Qtr Without Previous Qtr HIT: '
		,COUNT(DISTINCT(memid)) AS 'Unique Member Count' 
		FROM INFORMATICS.dbo.QA_PALLIATIVE_AUTH; */
...
FROM INFORMATICS.dbo.QA_PALLIATIVE_AUTH AS pall
	INNER JOIN INFORMATICS.dbo.uvw_BASELINE_MEMBERSHIP AS bm ON pall.memid = bm.memid

UNION ALL
SELECT pall.memid,bm.CIN
FROM INFORMATICS.dbo.QA_PALLIATIVE_CLAIM AS pall
	INNER JOIN INFORMATICS.dbo.uvw_BASELINE_MEMBERSHIP AS bm ON pall.memid = bm.memid
) -- CONCLUDE ...
AS baselinesource
-- WHERE 1=1
...
INTO INFORMATICS.dbo.TEMPLATE_PALLIATIVE
FROM #baselinememiso AS bm

 		SELECT ' ' AS 'MS EXCEL OLE DB OBDC (TEMPLATE): ',pall.* 
		FROM INFORMATICS.dbo.TEMPLATE_PALLIATIVE (NOLOCK) AS pall
		
		/* SELECT ' ' AS 'MS EXCEL OLE DB OBDC (CLAIMS): ',bm.*
		,pall.* 
		,[diag descr]
		FROM INFORMATICS.dbo.QA_PALLIATIVE_CLAIM (NOLOCK) AS pall
		...
		,pall.* 
		FROM INFORMATICS.dbo.QA_PALLIATIVE_AUTH (NOLOCK) AS pall
				LEFT JOIN INFORMATICS.dbo.uvw_BASELINE_MEMBERSHIP (NOLOCK) AS bm ON pall.memid = bm.memid */
-- =====================================================================
