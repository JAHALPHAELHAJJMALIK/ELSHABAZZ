...
-- C002: [TargetPopulation] ⏎ IN the 20240418 script, the POF assessment is updated to use the [ProviderPortal].ecm.[StratificationTargetPopulation] table instead, AS per the guidance FROM the ECM weekly teams meeting ON 20240418.
 ...
 -- C004: PER MEETING VIA TEAMS ON 20240509 WITH() MS KATHRYN + SIR JUAN BOTH TABLES ([MemberTargetPopulation] AND + & [StratificationTargetPopulation]) SHOULD BE USED TO DETERMINE ECM MEMBER [TargetPopulation] 
		...
		/* LEFT JOIN ProviderPortal.ecm.MemberTargetPopulation AS ECM_MT ON ECM_MT.MemberId = ECM.memid
...
FROM  #ECMEnrolled# AS ECM
	INNER JOIN ProviderPortal.ECM.Member AS ECM_M ON ECM.memid = ECM_M.memberid
...
FROM #ECMAdultTargetPopulation# AS ECM
ORDER BY ECM.memid
...
FROM #ECMPopulation# AS ECM
		LEFT JOIN #ECMAdultServices# AS ECMA ON ECMA.memid = ECM.memid
		LEFT JOIN #ECMChildServices# AS ECMP ON ECMP.memid = ECM.memid
...
FROM #ECMAdultandPeds# AS ECM
		LEFT JOIN ProviderPortal.ecm.MemberDisenrollment AS ECMD ON ECMD.MemberId = ECM.memid
		LEFT JOIN ProviderPortal.ecm.DisenrollmentReasons AS ECMDR ON ECMDR.DHCSCode = ECMD.Code
...
FROM #ECMpre# AS ECM
LEFT JOIN ECM_Prov1 ECMP ON ECMP.MemberId = ECM.memid
JOIN HMOPROD_PlanData.dbo.provider p ON ECMP.Providerid = p.provid
LEFT JOIN ProviderPortal.ECM.MemberOutreach AS ECMMem ON ECMMem.MemberId = ECM.memid
...
outer apply (SELECT DISTINCT ECM2.memid,  
			...
			FROM #ECMMemberOutreach# AS ECM2
