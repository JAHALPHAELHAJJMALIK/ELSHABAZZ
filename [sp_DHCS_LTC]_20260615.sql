...
CREATE  OR ALTER PROCEDURE dbo.[sp_DHCS_LTC] -- <Procedure_Name, sysname, ProcedureName> ... DROP PROCEDURE ... EXEC dbo.[sp_DHCS_LTC]; 
...
SET @lobplanprog = ISNULL(@LineofBusiness,'DSNP') -- SET @lobplanprog = 'DSNP MEDICARE PLAN' -- SELECT * FROM INFORMATICS.dbo.[uvw_LINE_OF_BUSINESS] -- ASSESS VALID ACTIVE OPTION(S)
...
FROM #AdmitAssessment AS aa
	...
	WHERE 1=1 
		AND c.claimid IN (SELECT DISTINCT claimid FROM INFORMATICS.dbo.DHCS_LTC_DSNP)
		) AS t
...
FROM #AdmitAssessment AS aa
	-- INNER JOIN INFORMATICS.dbo.DHCS_LTC_DSNP AS developsource ON aa.AdmitID = developsource.AdmitID
WHERE 1=1 
...
FROM INFORMATICS.dbo.DELIVERABLE_DHCS_LTC_DSNP AS s
	INNER JOIN INFORMATICS.dbo.DHCS_LTC_DSNP AS d ON s.memid = d.memid
WHERE 1=1
...
CROSS APPLY (SELECT DISTINCT [RANGE NOTE(S)] FROM INFORMATICS.dbo.DHCS_LTC_DSNP) AS rp -- INDEPENDENT STAND ALONE ... CROSS APPLY() CROSS INNER JOIN() CARTESIAN PRODUCT
WHERE 1=1
