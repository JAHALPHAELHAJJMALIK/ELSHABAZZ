-- =====================================================================
	...
	-- m.EnrollmentStatus = 'Assigned'), joined ON pof.memid = m.MemberId, matching the CODE DUB C 2.0
	-- supplied 20260714 --
	-- DUB C 2.0 confirmed the common field list 20260714: memid, carriermemid, lastname, firstname,
	...
	-- SELECT DISTINCT retained per DUB C 2.0 supplied CODE, guards against the mpa fan-out (2 to 3
	-- duplicate rows per member) observed IN DUB C 2.0 earlier ad-hoc exploration without a dedup/window
...
-- =====================================================================
	-- OPEN ITEMS -- 20260714 --
				/* 1) NO 'currently assigned' window OR dedup applied to mpa here, matching the supplied CODE exactly. Worth confirming whether the raw detail listing should also dedup duplicate provider-assignment rows before this goes to JAG, given DUB C 2.0 own exploration surfaced 2 to 3 duplicate mpa rows per member without such a window

				2) carriermemid, lastname, firstname, [RANGE NOTE(s)] are assumed present AND identically spelled across all 17 existing / 16 algorithm POF tables, per DUB C 2.0 confirmed field list. IF any one table's columns differ, that table's block above IS the one to correct */
-- =====================================================================
